...
public class MainActivity extends Activity implements NodeListener,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//setContentView(R.layout.activity_main_listitem_node); //added by caro 
	...
	private void initUsbDriver() {
		...
		try {
			...
			serialDriver.open(); //aca es donde no encuentra el usb y va al catch
			initStartTime = System.currentTimeMillis();
			zwaveManager.open();
		} catch (Exception x) { //The alertDialog was added by Caro
		    AlertDialog ad = new AlertDialog.Builder(this).create();  
		    ad.setCancelable(false); // This blocks the 'BACK' button  
		    ad.setMessage("No Z-wave stick found the application will finish");  
		    ad.setButton("OK", new DialogInterface.OnClickListener() {  
		        @Override  
		        public void onClick(DialogInterface dialog, int which) {  
		            dialog.dismiss();     
		            finish(); //This was the original code, no dialog, just the finish sentence.
		        }  
		    });  
		    ad.show();  
